...
public class ContDistEmpirical extends ContDist {
	...
	public Double sample() {
		...
		while (_values.get(i).entryFrequency < q) {
			i++; // try next item
		}
		...
		double newSample = lowVal
				+ (((highVal - lowVal) * (q - lowFreq)) / (highFreq - lowFreq));

		if (nonNegative && newSample < 0) {
			sendWarning(
					"You get a sample from a RealDistEmpirical distribution "
							+ "which is set to nonNegative. But the sample is negative!",
					"RealDistEmpirical: " + this.getName()
							+ " Method: public double sample() ",
					"The given distribution has negative values but all negative values "
							+ "should be ignored.",
					"Make sure not to set a RealDistEmpirical distribution with "
